        ...
        {actions.map((action) => (
          ...
          >
            <div className="flex items-center space-x-5">
              <div
                className={cn(
                  'flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl transition-all duration-300 shadow-sm',
                  action.color
                )}
              >
                {action.icon}
              ...
              <div className="flex flex-col text-left">
                <Text
                  variant="body"
                  className="font-black text-lg leading-none mb-1.5 group-hover:text-stellar-blue transition-colors"
                >
                  {action.label}
                </Text>
            ...
            <ArrowRight
              size={18}
              className="text-muted-foreground/40 group-hover:text-stellar-blue group-hover:translate-x-1 transition-all"
            />
          </button>
        ))}
      </CardContent>
